...
public class Matriz {
    ...
    public Matriz(int m, int n) {
      /*for (int i=0; i<datos.length; i++) //Columnas
            for(int k=0; k<datos[0].length; k++) //Filas
                datos[i][k]= k;*/
    ...
    public Matriz sumar(Matriz otra) {
      //if((datos.length != otra.datos.length) || (datos[0].length != otra.datos[0].length)){
	     //  return null;
	      //}
	    Matriz resSuma = new Matriz(datos.length, datos[0].length);
      for(int i=0; i<datos.length; i++){
        for(int j=0; j<datos[0].length; j++){
          resSuma.datos[i][j] = datos[i][j] + otra.datos[i][j];
        }
      }
      return resSuma;
    }
    ...
    public Matriz escalar(double c) {
      Matriz mulEsc = new Matriz(this.m, this.n);
      for(int i=0; i<this.m; i++){
          for(int j=0; j<this.n; j++) {
              mulEsc.datos[i][j] = this.datos[i][j]*c;
          }
      }
      return mulEsc;
    }
